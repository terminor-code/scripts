## Pyhton Script to help filter Nessus Csv for reporting
**`Workflow`**
...
# === Configuration ===
INPUT_FILE = 'test.xlsx'                    # Input Excel file
OUTPUT_FILE = 'final_merged_by_name.xlsx'   # Final output file

try:
    print("=== Step 1: Reading input Excel ===")
    df = pd.read_excel(INPUT_FILE)
    print(f"✅ Loaded {len(df)} rows from '{INPUT_FILE}'")

    # === Step 2: Check required columns ===
    required_cols = {'Risk', 'Host', 'Name', 'Synopsis', 'Description'}
    ...
    # === Step 6: For each host, keep the highest-risk row ===
    filtered_df = (
    ...
    # === Step 7: Merge again by Name (combine hosts per vuln) ===
